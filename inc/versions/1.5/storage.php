<?php
	...
	class WP_MANGA_STORAGE {
		...
		public function __construct() {
			...
			add_action( 'wp_manga_upload_after_extract', array( $this, 'flag_upload_start' ), 10, 5 );
				...
				if ( $dh = opendir( $extract ) ) {
					while ( ( $file = readdir( $dh ) ) !== false ) {
						...
						if ( '.' != $file && '..' != $file && strtolower( $file ) !== '.ds_store' ) {
							...
							$idx = strrpos($extract, "/");
							$image_folder_name = substr($extract, $idx + 1);
		 ...
		 **/
		function create_chapter( $chapter_args, $result, $storage, $overwrite = 'false', $extract = ''  ) {
		 ...
		 * $extract - folder name to store chapter images. If not passed, chapter slug will be used
		 **/
		function create_json( $post_id, $chapter = array(), $result, $storage, $overwrite = 'false', $extract = '' ) {
			global $wp_manga;
			...
			// since 1.5, we md5() folder name at first time, so there is no need to change folder name each time we edit chapter name
			/*
			if ( ! empty( $chapter['new_slug'] ) ) {
				...
				if ( isset( $data['chapters'][ $chapter_id ]['storage']['local'] ) ) {
					...
					if ( file_exists( $chapter_dir ) ) {
						...
						if($extract == '') {
							// change new image folder name
							$extract = $new_chapter_dir;
						}
					}
			...
			*/
			...
			foreach ( $result['file'] as $file ) {
				...
				if($image_folder_name != '') {
					$file = str_replace($chapter_slug, $image_folder_name, $file);
				} else {
			...
			$data = apply_filters( 'madara_manga_data_json', $data, $post_id, $chapter_id, $storage );
		...
		function duplicate_server( $post_id, $chapter_id, $duplicate_to ) {
			...
			if ( isset( $chapter['storage']['local'] ) ) {
				...
				if ( $duplicate_to !== 'local' ) {
					$chapter_dir = WP_MANGA_EXTRACT_DIR . $uniqid . '/' . $chapter_slug . '/';
					$chapter_uri = WP_MANGA_EXTRACT_URL . $uniqid . '/' . $chapter_slug . '/';
				}
			...
			if ( file_exists( $extract ) ) {
				...
				if ( is_wp_error( $result ) || isset( $result['error'] ) || empty( $result ) ) {
					...
					if ( $overwrite == 'true' ) {
						...
						$chapter_id = $this->create_chapter( $chapter_args, $result, $storage, $overwrite, $extract );
					}
		...
		function manga_upload( $post_id, $manga_zip, $storage ) {
			...
			foreach ( $scandir_lv1 as $dir_lv1 ) {
				...
				if ( is_dir( $dir_lv1 ) ) {
					...
					$dir_slug_lv1       = md5(basename( $dir_lv1 ) . date("Y-m-d H:i:s"));
					...
					foreach ( $scandir_lv2 as $dir_lv2 ) {
						...
						if ( is_dir( $dir_lv2 ) ) {
							...
							$dir_slug_lv2   = md5(basename( $dir_lv2 ) . date("Y-m-d H:i:s"));;
		...
		function flag_upload_start( $post_id, $extract, $extract_uri, $storage ) {
